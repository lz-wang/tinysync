...
总体进度与当前优先级见 [ROADMAP.md](../../ROADMAP.md)。本页勾选项表示仓库中已有实现或配置；远端发布链路以实际验收记录为准。
...
- [x] 核实 `v0.1.0` Release workflow、发行资产与校验和，并记录远端验收结果（2026-09-15 核对：GitHub Release `v0.1.0` 已于 2026-09-14 发布，六平台发行档案与 `checksums.txt` 齐全）。
- [ ] 独立验收 WebDAV 镜像结果；镜像成败不能由 GitHub Release 成功推导。
