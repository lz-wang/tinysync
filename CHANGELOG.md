...
- 本地文件支持共享（Share）：除单文件外，可直接共享目录与整个同步任务的本地根；共享名称可选且即 URL slug（留空生成随机标识），支持过期与启停；创建入口位于本地文件页的行内按钮与「共享当前目录」工具栏按钮。
- 新增公开访问页 `/shared`：以卡片列出全部可访问的共享（名称、分享时间、过期时间），过期与禁用的共享完全隐藏；`/shared/<slug>` 浏览页复用文件管理器交互，提供目录下钻、分页、文件下载与复制链接，公开侧始终隐藏点文件。
- 新增无认证公开 API：`GET /api/v1/public/shares`（索引卡片）与 `GET /api/v1/public/shares/:slug/entries`（目录分页浏览）；共享文件直链形如 `/shared/<slug>/<file>`，支持 Range / HEAD，页面与直链带 noindex。
- 新增 TinySync SVG 产品图标并用于浏览器标签页；标签页标题随页面、文件视图与目录、设置标签及运行详情任务名称自动更新。
- 文件菜单更名为“文件管理”；远端、本地与已发布视图及当前浏览目录现在同步到浏览器地址。远端与本地浏览改用统一的固定高度文件管理器布局，提供位置栏、面包屑、刷新、分页表格和下载，目录列表在内部滚动；默认隐藏点开头的文件和文件夹，可在刷新按钮左侧切换显示；除居中操作列外，文件表格列均左对齐并支持当前目录排序；本地状态列显示受管理归属，可从居中的纯图标“分享”操作进入发布，远端无法提供的文件属性以 `-` 显示。
- 同步源与同步任务页面改用全宽、固定可视区域高度的可搜索、筛选、排序、分页和多选批量操作表格；项目较多时仅表格内容区滚动。关键删除操作增加确认。同步源连接测试改为 toast 提示，任务最近运行结果可直接进入运行详情。
...
- SFTP 同步源的主机密钥 SHA256 指纹改为可选项；填写时严格校验，留空时跳过校验，且该字段移动到表单末尾。
- 同步源编辑面板将“启用”开关固定在底部操作栏，滚动表单时始终可见。
- 创建同步任务时可浏览并选择运行 TinySync 主机上的本地目录；直接填写不存在的路径会在创建前自动建立，失败时返回具体错误。
- 本地目录选择器固定为 50% 页面宽度与 80% 页面高度，默认从运行用户 Home 目录开始；支持显示隐藏目录及在当前目录直接新建文件夹。
- 远端根目录选择器现复用本地目录选择器的固定尺寸与操作方式，默认隐藏点开头条目，并支持在当前远端目录直接新建文件夹。
- 同步任务编辑面板按关联字段重排为双列布局，统一远端与本地目录浏览操作，并将启用状态固定在底部操作栏。
- 同步源编辑面板按协议重排字段：WebDAV 与 SFTP 支持配置远端根目录，SFTP 留空时使用登录用户 Home；S3 服务地址、Access Key 与 Secret Key 改为必填并采用标准术语。
- 设置页改为带图标的顶部标签导航：帐号资料与安全设置归入“帐号设置”，开发者标签直接管理 API Token，且标签切换会同步浏览器地址。
...
- 破坏性变更：移除 v0.6 的「发布」功能与全部相关端点（`/api/v1/published-files`、`/published/*path`），由共享（`/api/v1/shares`、`/shared`）取代，不留兼容别名；升级时 `published_files` 表被废弃且旧发布策略不迁移，升级后需重新创建共享。
