...
- 新增仅凭唯一 slug 访问的公开共享页 `/shared/<slug>`：复用文件管理器交互，提供目录下钻、分页、文件下载与复制链接，公开侧始终隐藏点文件；裸 `/shared` 重定向至主页，公开共享枚举 API 不提供。
- 新增无认证公开 API：`GET /api/v1/public/shares/:slug/entries`（目录分页浏览）；共享文件直链形如 `/shared/<slug>/<file>`，支持 Range / HEAD，页面与直链带 noindex。
